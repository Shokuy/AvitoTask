...
Параметры: ${JSON.stringify(item.params || {})}`;
    ...
    if (type === "description") {
      ...
      systemPrompt = "Ты — эксперт по ценообразованию на площадке Авито. Анализируй данные товара и давай краткую рекомендацию по рыночной цене на русском языке. Ответ должен быть компактным: одна строка со средней ценой, затем 2-3 строки с ценовыми диапазонами в формате «цена — пояснение». Не используй markdown. Не пиши длинных объяснений.";
